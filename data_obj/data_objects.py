...
#
# def sample_by_group(full: pa.Table, group_cols, k=100,
#                           replace="auto",
#                           rng: np.random.Generator | None = None) -> pa.Table:
#     """
#     Arrow-native: enumerate groups via TableGroupBy, make a boolean mask per group,
#     sample row indices, take once at the end.
#     """
#     if rng is None:
#         rng = np.random.default_rng()
#
#     # Normalize chunking once; keeps schema/order and makes boolean→NumPy clean.
#     full = full.combine_chunks()
#
#     # 1) Enumerate unique group combinations (TableGroupBy)
#     # We don't actually need an aggregate; any aggregate returns one row per group.
#     # Count is cheap and handy for sanity checks.
#     gb = pa.TableGroupBy(full, group_cols)
#     groups = gb.aggregate([(group_cols[0], "count")]).drop_columns([f"{group_cols[0]}_count"])
#
#     # 2) Helper: mask full table to rows matching one group row
#     def group_mask(row_i: int) -> pa.ChunkedArray:
#         m = None
#         for c in group_cols:
#             val = groups[c][row_i].as_py()
#             eq = pc.equal(full[c], val)  # Kleene-equality handles nulls
#             m = eq if m is None else pc.and_kleene(m, eq)
#         return m  # boolean ChunkedArray (single-chunk after combine)
#
#     # 3) For each group: get row indices, sample, collect
#     take_idx_parts = []
#     for i in range(groups.num_rows):
#         m = group_mask(i)
#         idx = np.flatnonzero(m.to_numpy(zero_copy_only=False))
...
#         if n == 0:
#             continue
#         do_replace = (n < k) if replace == "auto" else bool(replace)
#         if do_replace:
#             sel = rng.choice(idx, size=k, replace=True)
#         else:
#             kk = min(k, n)
#             sel = rng.choice(idx, size=kk, replace=False)
#         take_idx_parts.append(sel)
#
#     if not take_idx_parts:
#         return full.slice(0, 0)
#
#     take_idx = np.concatenate(take_idx_parts)
#     return full.take(pa.array(take_idx, type=pa.int64()))

# def sample_by_group(full: pa.Table, group_cols, k=100, replace="auto", rng=None) -> pa.Table:
#     if rng is None:
#         rng = np.random.default_rng()
#
#
...
#         # For parity with your dict, expose maxrho as the vector from the best window
#         'maxrho': best_rhos,
#         'annotation': _as_lenN_array(annotation, len(max_rhos))
#         # annotations#_repeat_scalar(annotation, sample_size, pa.string()),
#     }
#     # replicate gd for each row
#     for k, v in gd.items():
#         cols_full[k] = _as_lenN_array(get_static(v), len(max_rhos))
...
#
#
#     # 1) sort by group cols
#     sort_idx = pc.sort_indices(full, sort_keys=[(c, "ascending") for c in group_cols])
#     full_sorted = full.take(sort_idx).combine_chunks()  # now every column is a single Array
#
#     # 2) struct key from Arrays (not ChunkedArrays)
#     arrays = [full_sorted.column(c).chunk(0) for c in group_cols]  # each is pa.Array now
#     keys_struct = pa.StructArray.from_arrays(arrays, names=group_cols)
#
#     # 3) group boundaries via run-end encode
#     ree = pc.run_end_encode(keys_struct)
#     run_ends = np.asarray(ree.run_ends)
#     starts = np.concatenate(([0], run_ends[:-1]))
#     lengths = run_ends - starts
#
#     # 4) sample k per group
#     take_sorted_offsets = []
...
#         do_replace = (n < k) if replace == "auto" else bool(replace)
#         if do_replace:
#             sel = rng.integers(start, start + n, size=k, dtype=np.int64)
#         else:
#             kk = min(k, n)
#             sel = start + rng.choice(n, size=kk, replace=False)
#         take_sorted_offsets.append(sel)
#
#     if not take_sorted_offsets:
#         return full.slice(0, 0)
#
#     take_sorted_offsets = np.concatenate(take_sorted_offsets)
#
#     # 5) map back to original row indices
#     original_row_idx = sort_idx.to_numpy()[take_sorted_offsets]
#
#     # 6) return sampled rows
#     return full.take(pa.array(original_row_idx, type=pa.int64()))


# def sample_by_group(full: pa.Table,
#                     group_cols,
#                     k: int = 100,
#                     replace: str = "auto",   # "auto" | True | False
#                     rng: np.random.Generator | None = None) -> pa.Table:
#     """
#     Return a table with exactly k samples per group (if possible).
#     Groups are defined by `group_cols`.
#
#     replace="auto": sample without replacement if group_size>=k, else with replacement.
#     replace=True/False: force behavior.
#     """
#     if rng is None:
#         rng = np.random.default_rng()
#
#     # 1) Build a struct key for grouping
#     keys_struct = pa.StructArray.from_arrays([full[c] for c in group_cols], names=group_cols)
#
#     # 2) Sort rows by the group key so each group is a contiguous run
#     sort_idx = pc.sort_indices(keys_struct)
#     full_sorted = full.take(sort_idx)
#     keys_sorted = keys_struct.take(sort_idx)
#
#     # 3) Run-end encode the sorted keys to get group boundaries
#     ree = pc.run_end_encode(keys_sorted)  # RunEndEncodedArray
#     run_ends = np.asarray(ree.run_ends)   # shape (num_groups,)
#     starts = np.concatenate(([0], run_ends[:-1]))
#     lengths = run_ends - starts
#
#     # 4) For each group, pick indices within [start, end)
#     take_offsets = []
#     for start, length in zip(starts, lengths):
#         n = int(length)
#         if n == 0:
...
#             sel = rng.integers(start, start + n, size=k, dtype=np.int64)
#         else:
#             # without replacement, limited by group size
#             kk = min(k, n)
#             sel = start + rng.choice(n, size=kk, replace=False)
#
#         take_offsets.append(sel)
#
#     if not take_offsets:
#         # no groups? return empty slice with same schema
#         return full.slice(0, 0)
#
#     take_offsets = np.concatenate(take_offsets)
#
#     # 5) Map from offsets-in-sorted to original row indices
#     sorted_idx_np = sort_idx.to_numpy()
#     original_row_idx = sorted_idx_np[take_offsets]
#     print(original_row_idx)
#     # 6) Take from the original table (keeps original schema/col order)
#     return full.take(pa.array(original_row_idx, type=pa.int64()))
import itertools
...
# class GroupConfig(RunConfig):
#     def __init__(self, grp_specs):
#         iterable_d = {}
#         if isinstance(grp_specs, dict):
#             iterable_d = {k: correct_iterable(v) for k, v in grp_specs.items()}
#         elif isinstance(grp_specs, RunConfig):
#             iterable_d = {k: correct_iterable(v) for k, v in grp_specs.__dict__.items()}
#
#         super().__init__(iterable_d)
#         self.output_path = None
#         self.output = None
#
#     def pull_output(self, to_table=True, summary=True, full=False):
#         if self.output_path is None or len(self.output_path) == 0:
#             print('no output path specified')
#             return
#
...
#             return filtered_table
#         else:
#             self.output = Output(filtered_table, path=file_path)
#
#     def trait_hierarchy(self, full_ds, trait, level="below", threshold=0.9, include_ids=False):
#         """
#         Return traits that are above or below the grouping level of a given trait.
...
class DataGroup:
    ...
    #     grp_config = self.get_group_config()#GroupConfig({**self.static_traits, **self.nonstatic_traits, **self.internal_traits})
    #     # grp_config.output = self.output
    #     output = self.output
    #     output.grp_config = grp_config
    #     return output
...
class Output:
    ...
    # def add_surrogate_relation_cat(self):
    #     filtered_table = self._full
    #     if 'relation' in filtered_table.schema.names:
    #         filtered_table = filtered_table.append_column('relation_cat', filtered_table['relation'])
    #
    #     if 'surr_var' in filtered_table.schema.names:
    # def aggregate_libsize(self, query_config=None): #process_group_table
    #     knn = get_static(query_config.knn if query_config is not None else self.grp_config)
    #     if "LibSize" in self.full.schema.names:
    #         mask = pc.greater(self.full["LibSize"], knn+1)
    #         group_table = self.full.filter(mask)
    #
    #     if group_table.num_rows == 0:
    #         return None
    #
    #     calc_grp_cols = [col for col in self.grp_config.traits if (col in self.full.schema.names) and (col not in "output_path")]
    #     if "LibSize" in self.full.schema.names:
    #         if "LibSize" not in calc_grp_cols:
    #             calc_grp_cols.append("LibSize")
    #     if 'relation' in self.full.schema.names:
    #         if 'relation' not in calc_grp_cols:
    #             calc_grp_cols.append('relation')
    #
    #     aggregated_cols = [col for col in self.full.schema.names if (col not in calc_grp_cols) and ('id' not in col) and ('ind' not in col) and (self.full[col].type in [pa.float32(), pa.float64(), pa.int32(), pa.int64()])]
    #     grouped_aggregated_table = pa.TableGroupBy(self.full, calc_grp_cols).aggregate([(col, "mean") for col in aggregated_cols])
    #     self.active = grouped_aggregated_table

    # def separate_real_surr(self):
    #     table = self.active
    #     # or run separate_real_surr(table) from a utility module
    #     if 'surr_var' in table.schema.names:
    #         mask = pc.equal(table['surr_var'], 'neither')
    #         real_table = table.filter(mask)
    #         surr_table = table.filter(pc.invert(mask))
...
# oc.table --> table
# oc.table.get_real() --> real table
# oc.table.get_surrogate() --> surrogate table

# output_collection = {f1_label : f1(full)}
...
class OutputCollection:
    ...
    # def get_metrics(self, relationship_id='r1'):
    #     return

    # def calc_top_vals(self, relationship, relationship_id='r1'):
    #
    #     gb_real = self.delta_rho_stats.real.group_by(["relation", 'lag', 'surr_var', 'surr_num']).aggregate([("maxlibsize_rho", "mean"), ("delta_rho", "mean")])
    #     gb_real_df = gb_real.to_pandas()
    #
    #     gb_surr = self.delta_rho_stats.surrogate.group_by(["relation", 'lag', 'surr_var', 'surr_num']).aggregate([("maxlibsize_rho", "mean")])
    #     gb_surr_df= gb_surr.to_pandas()
    #
    #     # rel_obj = Relationship(var_x = 'temp')
    #     # for rel in (relationship.surr_r1x, relationship.surr_r1y
    #     if relationship_id == 'r1':
    #         real_r1_df = gb_real_df[(gb_real_df['relation']==relationship.r1) & (gb_real_df['surr_var']=='neither')].reset_index(drop=True)
    #         real_r1_ind = np.argmax(real_r1_df['maxlibsize_rho_mean'].values)
    #         print('real_r1_ind', real_r1_ind)
    #         real_r1_d = real_r1_df.iloc[real_r1_ind].to_dict()
    #         print('real_r1_d', real_r1_d)
    #
    #         surr_r1x_df = gb_surr_df[(gb_surr_df['relation']==relationship.r1) & (gb_surr_df['surr_var']==relationship.var_x)]
    #         surr_r1x_count = len(surr_r1x_df.surr_num.unique())
    #         surr_r1x_df_outperformers = surr_r1x_df[surr_r1x_df['maxlibsize_rho_mean'] > real_r1_d['maxlibsize_rho_mean']]
    #         surr_r1x_df_outperformers_count = len(surr_r1x_df_outperformers.surr_num.unique())
    #
    #         surr_r1y_df = gb_surr_df[
    #             (gb_surr_df['relation'] == relationship.r1) & (gb_surr_df['surr_var'] == relationship.var_y)]
    #         surr_r1y_count = len(surr_r1y_df.surr_num.unique())
    #         surr_r1y_df_outperformers = surr_r1y_df[surr_r1y_df['maxlibsize_rho_mean'] > real_r1_d['maxlibsize_rho_mean']]
    #         surr_r1y_df_outperformers_count = len(surr_r1y_df_outperformers.surr_num.unique())
    #
    #
    #         self.surr_r1x_count = surr_r1x_count
    #         self.surr_r1x_count_outperforming = surr_r1x_df_outperformers_count
    #         self.surr_r1y_count = surr_r1y_count
    #         self.surr_r1y_count_outperforming = surr_r1y_df_outperformers_count
    #         self.r1_deltarho = real_r1_d['delta_rho_mean']
    #         self.r1_maxlibsize_rho = real_r1_d['maxlibsize_rho_mean']
    #         self.r1_lag = real_r1_d['lag']
    #
    #     elif relationship_id == 'r2':
    #         real_r2_df = gb_real_df[(gb_real_df['relation']==relationship.r2) & (gb_real_df['surr_var']=='neither')].reset_index(drop=True)
    #         real_r2_ind = np.argmax(real_r2_df['maxlibsize_rho_mean'].values)
    #         real_r2_d = real_r2_df.iloc[real_r2_ind].to_dict()
    #
    #         surr_r2x_df = gb_surr_df[
    #             (gb_surr_df['relation'] == relationship.r2) & (gb_surr_df['surr_var'] == relationship.var_x)]
    #         surr_r2x_count = len(surr_r2x_df.surr_num.unique())
    #         surr_r2x_df_outperformers = surr_r2x_df[surr_r2x_df['maxlibsize_rho_mean'] > real_r2_d['maxlibsize_rho_mean']]
    #         surr_r2x_df_outperformers_count = len(surr_r2x_df_outperformers.surr_num.unique())
    #
    #         surr_r2y_df = gb_surr_df[
    #             (gb_surr_df['relation'] == relationship.r2) & (gb_surr_df['surr_var'] == relationship.var_y)]
    #         surr_r2y_count = len(surr_r2y_df.surr_num.unique())
    #         surr_r2y_df_outperformers = surr_r2y_df[surr_r2y_df['maxlibsize_rho_mean'] > real_r2_d['maxlibsize_rho_mean']]
    #         surr_r2y_df_outperformers_count = len(surr_r2y_df_outperformers.surr_num.unique())
    #
    #         self.r2_deltarho = real_r2_d['delta_rho_mean']
    #         self.r2_maxlibsize_rho = real_r2_d['maxlibsize_rho_mean']
    #         self.surr_r2x_count =  surr_r2x_count
    #         self.surr_r2x_count_outperforming = surr_r2x_df_outperformers_count
    #         self.surr_r2y_count = surr_r2y_count
    #         self.surr_r2y_count_outperforming = surr_r2y_df_outperformers_count
    #         self.r2_lag = real_r2_d['lag']
    #
...
class DataVarConfig:
    def __init__(self, config, var_id, proj_home, suffix_label=None, suffix_ind=None):
    ...
    def populate(self, config, proj_home):
        try:
            ...
            self.load_from_config( config, proj_home)

    def load_from_var_yaml(self, var_yaml, proj_dir):
        print('load_from_var_yaml function is a stub - needs to be implemented')
        pass

    def load_from_config(self, config, proj_home):

        proj_dir = Path(proj_home)/config.proj_name
        var_id = self.var_id
        ...
        self.raw_data_dir_path = self.set_data_source(config, proj_dir, data_source='data', data_type='raw')
        self.get_color(config)
        self.set_surr_csv_name()
        self.surr_data_dir_path = self.set_data_source(config, proj_dir, data_source='data', data_type='surr')
    ...
    def set_data_source(self, config, proj_dir,data_source='data' , var_data_csv=None, data_type='raw'):
        ...
        data_path, _ = choose_data_source(proj_dir, config, data_source, data_type=data_type, var_data_csv=var_data_csv)
...
class VarObject(DataVarConfig):
    def __init__(self, config, var_id=None, proj_home=None, data_var_config=None):
        if data_var_config is not None and isinstance(data_var_config, DataVarConfig):
            ...
            super().__init__(config, var_id, proj_home)
        ...
        self.col_name = None
        self.time_var = None
        self.proj_dir = Path(proj_home)/config.proj_name
    ...
    def get_surr(self, surr_num):
        ...
        if (self.surr_data_dir_path / check_csv(self.surr_csv)).exists() is True:
            ...
            self.col_name = f'{self.surr_prefix}_{surr_num}'
...
class CCMConfig(RunConfig):
    ...
    def __init__(self, grp_specs, config):
        iterable_d = {}
        if isinstance(grp_specs, dict):
            iterable_d = {k: correct_iterable(v) for k, v in grp_specs.items()}
        elif isinstance(grp_specs, RunConfig):
        ...
        self.output_path = self.set_file_dir(config, self.output_dir, self.file_name)
    ...
    def check_run_exists(self):
        ...
        check_exists(check_csv(self.file_name), Path(self.output_path))
        if self.output_dir is None or self.file_name is None:
            return False
        output_path = Path(self.output_dir)/check_csv(self.file_name)
        return output_path.exists()

    def set_file_dir(self, config, output_dir, file_name):
        ...
        self.output_path = grp_path

    def set_col_ts(self, surr_num=None):
        if surr_num is None:
            self.col_var_obj.get_raw()
        else:
            self.col_var_obj.get_surr(surr_num)

    def set_target_ts(self, surr_num=None):
        if surr_num is None:
            self.target_var_obj.get_raw()
        else:
            self.target_var_obj.get_surr(surr_num)


    def make_df(self):

        col_df = self.col_var_obj.ts.rename(columns={self.col_var_obj.col_name: self.col_var})
        target_df = self.target_var_obj.ts.rename(columns={self.target_var_obj.col_name: self.target_var})
...
class Relationship:
    ...
    # def calc_metrics(self, real_out, surr_out, relationship_id='r1'):
    #
    #     gb_real = real_out.delta_rho_stats.real.group_by(["relation", 'lag', 'surr_var', 'surr_num']).aggregate([("maxlibsize_rho", "mean"), ("delta_rho", "mean")])
    #     gb_real_df = gb_real.to_pandas()
    #
    #     gb_surr = surr_out.delta_rho_stats.surrogate.group_by(["relation", 'lag', 'surr_var', 'surr_num']).aggregate([("maxlibsize_rho", "mean")])
    #     gb_surr_df= gb_surr.to_pandas()
    #
    #     # rel_obj = Relationship(var_x = 'temp')
    #     # for rel in (relationship.surr_r1x, relationship.surr_r1y
    #
    #     if relationship_id == 'r1':
    #         relationship = self.r1
    #     elif relationship_id == 'r2':
    #         relationship = self.r2
    #
    #     real_r_df = gb_real_df[(gb_real_df['relation']==relationship) & (gb_real_df['surr_var']=='neither')].reset_index(drop=True)
    #     real_r_ind = np.argmax(real_r_df['maxlibsize_rho_mean'].values)
    #     real_r_d = real_r_df.iloc[real_r_ind].to_dict()
    #
    #     surr_rx_df = gb_surr_df[(gb_surr_df['relation']==relationship) & (gb_surr_df['surr_var']==self.var_x)]
    #     surr_rx_count = len(surr_rx_df.surr_num.unique())
    #     surr_rx_df_outperformers = surr_rx_df[surr_rx_df['maxlibsize_rho_mean'] > real_r_d['maxlibsize_rho_mean']]
    #     surr_rx_df_outperformers_count = len(surr_rx_df_outperformers.surr_num.unique())
    #
    #     surr_ry_df = gb_surr_df[
    #         (gb_surr_df['relation'] == relationship) & (gb_surr_df['surr_var'] == self.var_y)]
    #     surr_ry_count = len(surr_ry_df.surr_num.unique())
    #     surr_ry_df_outperformers = surr_ry_df[surr_ry_df['maxlibsize_rho_mean'] > real_r_d['maxlibsize_rho_mean']]
    #     surr_ry_df_outperformers_count = len(surr_ry_df_outperformers.surr_num.unique())
    #
    #     if relationship_id == 'r1':
    #         self.surr_r1x_count = surr_rx_count
    #         self.surr_r1x_count_outperforming = surr_rx_df_outperformers_count
    #         self.surr_r1y_count = surr_ry_count
    #         self.surr_r1y_count_outperforming = surr_ry_df_outperformers_count
    #         self.r1_deltarho = real_r_d['delta_rho_mean']
    #         self.r1_maxlibsize_rho = real_r_d['maxlibsize_rho_mean']
    #         self.r1_lag = real_r_d['lag']
    #
    #     elif relationship_id == 'r2':
    #         # real_r2_df = gb_real_df[(gb_real_df['relation']==relationship.r2) & (gb_real_df['surr_var']=='neither')].reset_index(drop=True)
    #         # real_r2_ind = np.argmax(real_r2_df['maxlibsize_rho_mean'].values)
    ...
    #         # surr_r2y_df_outperformers_count = len(surr_r2y_df_outperformers.surr_num.unique())
    #
    #         self.r2_deltarho = real_r_d['delta_rho_mean']
    #         self.r2_maxlibsize_rho = real_r_d['maxlibsize_rho_mean']
    #         self.surr_r2x_count =  surr_rx_count
    #         self.surr_r2x_count_outperforming = surr_rx_df_outperformers_count
    #         self.surr_r2y_count = surr_ry_count
    #         self.surr_r2y_count_outperforming = surr_ry_df_outperformers_count
    #         self.r2_lag = real_r_d['lag']
...
#
#     def setup(self):
#         self.output_path = Path(self.config.get_dynamic_attr("output.{var}", "output_path"))
#         self.output_path.mkdir(parents=True, exist_ok=True)
#
#         grp_specs_list = self.config.get_dynamic_attr("group_specs.{var}", "group_specs_list")
#         if grp_specs_list is None or len(grp_specs_list) == 0:
#             raise ValueError("No group specifications found in configuration.")
#
#         for grp_specs in grp_specs_list:
#             if isinstance(grp_specs, dict):
#                 data_group = DataGroup(grp_specs)
#                 data_group.get_files(self.config, self.output_path)
#                 self.data_groups.append(data_group)
#             else:
#                 raise ValueError("Each group specification must be a dictionary.")
#
#     def pull_all_outputs(self, summary=True, full=False):
#         for data_group in self.data_groups:
#             data_group.pull_output(summary=summary, full=full)
